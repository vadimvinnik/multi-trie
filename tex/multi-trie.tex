...
The notion of naming and its most general formalisation, taken as a sterting
point, has been transformed into a specialisation enriched with specifics of
a \emph{structured} object and that of a many-valued object. Being structured
means that a name in an object can not only refer to an atomic value but also
...
It is not surprising that structuredness and manyvaluedness appear together in
this consideration. Although univalued structured objects seem more familiar
(and, in fact, found in practical programming everywhere), it is multivaluedness
that makes mathematical properties of such namings so harmonic: otherwise
set-theoretical union and intersection, cartesian product, flattening and
application would not have sense. Due to multivaluedness, these operations are
not just meaningful but also obey `good' properties, e.g.
associativity and/or distributivity over other operations.
...
represent a non-deterministic structure unfolding when the consumer
...
list is a normal contents. A typical example could be a directory tree
