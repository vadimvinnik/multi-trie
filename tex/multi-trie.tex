...
\[
  ...
  \} .
\]
The empty name~$\varepsilon$ has in~$p$ two values, namely~$q_1$ and~$q_2$.
...
\varepsilon = \varepsilon$ in~$\flatten(p)$.  A name~$a$ has one value in~$q_1$
...
Now look at the name~$aa$. Its value~3 is inherited from~$q_1$. Name~$a$
in~$p$ has a valaue~$q_3$ that, in its turn, contains a name~$a$ with a
value~5. Hence, the name~$aa$ has in $\flatten(p)$ the second value~5.
\end{Ex}
