...
Let $p\in \setmvcn{V}{D\to D'}$, $q\in \setmvcn{V}{D}$. Then \emph{cartesian
...
potentially useful properties.  The following two application operations are
...
\begin{eqnarray*}
  ...
  & b_1(v) = \apply_{\setcharmt}^{\odot}(f(v), q) .
\end{eqnarray*}
\end{Df}
...
\item for every atomic name~$v\in V$, apply the entire multi-trie~$p$ of
...
be~$b_2(v)$;
\item merge $b_1(v)$ and $b_2(w)$ with the respective multi-trie operation
(union or intersection), let $b_1(v)\odot b_2(v) = b(v)$;
\item compose the resulting multi-trie from the set~$a$ and a mapping~$b$ that
maps atomic names to child multi-tries.
\end{itemize}
