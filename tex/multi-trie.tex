...
\[
  (\forall u\in V^\ast . \deref(s,u) = \deref(t,u)) \implies (s = t) .
\]
\end{St}
...
Let~$s \in \setmvcn{V}{D}$, $u, v \in V^\ast$, $s, t \in 2^D$. Then
\begin{eqnarray*}
  & \deref(\putval(s, u, s), u) = s , \\
  & u \neq v \implies \deref(\putval(s, u, s), v) = \deref(s, v) , \\
  & \putval(\putval(s, u, s), u, t) = \putval(s, u, t) , \\
  & u \neq v \implies \putval(\putval(s, u, s), v, t) = \putval(\putval(s, v, t), u, s) .
\end{eqnarray*}
\end{St}
...
For any~$s,t\in\setmvcn{V}{D}$, $u, v\in V^\ast$, $\odot\in\{\cup, \cap\}$,
\begin{eqnarray*}
  & \select(\bot,u) = \bot, \\
  & \select(\top,u) = \top, \\
  & \select(s,\varepsilon) = s, \\
  & \select(s,uv) = \select(\select(s,u), v), \\
  & \select(s\odot t, u) = \select(s,u)\odot \select(t,u).
\end{eqnarray*}
\end{St}
In other words,
\begin{itemize}
