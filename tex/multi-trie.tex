...
compound name as a path in the trie, and gives a sub-trie starting
at this node.
...
$p = (s, f)$, $q = (t, g)$. Then
\[
  p \odot_\setcharmt  q = (s \odot t, h) ,
\]
where~$h$ is such a function that $h(v) = f(v) \odot_\setcharmt  g(v)$,
for any~$v \in V$.
...
In other words, to build $r = p \cup q$, one needs to
build a union of the value sets contained in the operands under
each compound name.
...
multitries. The former has higer abstraction level and suits well for
...
if it associated this name with an empty multitrie.
...
the multitrie data type an instance of applicative functor and monad
...
a multitrie could represent a static entity for which a tree of lists is a
